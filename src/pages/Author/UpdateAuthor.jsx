...
export default function UpdateAuthor() {
  ...
  useEffect(() => {
    if (!id) return; // tunggu authors selesai load
    const fetchauthor = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://127.0.0.1:9000/api/authors/${id}`);
        ...
        setErrors({ api: "Terjadi kesalahan saat memuat Penulis", err });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = id
        ? `http://127.0.0.1:9000/api/authors/${id}`
        : "http://127.0.0.1:9000/api/authors";
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {id ? "Edit Penulis" : "Tambah Penulis Baru"}
          </h1>
          <p className="text-gray-600">
            Lengkapi informasi Penulis yang{" "}
